...
Player::Player(std::string playerTexturePath, SDL_Rect *cropPNG, SDL_Rect *positionPNG, SDL_Renderer *renderer){
	   // Query attributes of texture
     SDL_Texture* playerTexture = loadFiles(playerTexturePath, renderer);
...
void Player::move(float change, const Uint8 *keyState, SDL_Rect *cropPNG, SDL_Rect *positionPNG){
...
void Player::render(SDL_Rect *cropPNG, SDL_Rect *positionPNG, SDL_Renderer *renderer, SDL_Rect *cam) {
    SDL_Rect tempRect;
    ...
    printf("Trying to do SDL_RenderCopy\n");

    SDL_RenderCopy(renderer, playerTexture, cropPNG, &tempRect);

    printf("And it never gets here\n");
}
