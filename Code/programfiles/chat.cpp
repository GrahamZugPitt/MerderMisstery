...
void enter_chat(SDL_Event e, bool *quit, const Uint8 *keyState, SDL_Renderer* renderer){
  ...
  //TTF_RenderText_Blended could only be used on SDL_Surface then you have to create the surface first
  // SDL_Surface* surfaceInputText = TTF_RenderText_Blended_Wrapped(fontStyle, inputText.c_str(), textWhite, 1215);
  ...
  //Rerender text if needed
    /*if(renderText) {
      ...
      if(inputText != "") {
        ...
        surfaceInputText = TTF_RenderText_Blended(fontStyle, inputText.c_str(), textWhite);
        textureInputText = SDL_CreateTextureFromSurface(renderer, surfaceInputText);
      }
      //Text is empty
      else {
        surfaceInputText = TTF_RenderText_Blended(fontStyle, " ", textWhite);
        textureInputText = SDL_CreateTextureFromSurface(renderer, surfaceInputText);
      }
      SDL_QueryTexture(textureInputText, NULL, NULL, &textW, &textH);
      inputRect.w = textW;
